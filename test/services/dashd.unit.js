...
var QuantsisnetService = proxyquire('../../lib/services/quantisnetd', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultQuantsisnetConf = fs.readFileSync(path.resolve(__dirname, '../data/default.quantisnet.conf'), 'utf8');

describe('Quantsisnet Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var quantisnetd = new QuantsisnetService(baseConfig);
      should.exist(quantisnetd);
    });
    it('will create an instance without `new`', function() {
      var quantisnetd = QuantsisnetService(baseConfig);
      should.exist(quantisnetd);
    });
  ...
  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      QuantsisnetService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      ...
      config.should.equal(defaultQuantsisnetConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a quantisnet.conf file', function() {
      var TestQuantsisnet = proxyquire('../../lib/services/quantisnetd', {
        fs: {
          readFileSync: readFileSync,
      ...
      var quantisnetd = new TestQuantsisnet(baseConfig);
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var quantisnetd = new TestQuantsisnet(config);
  ...
  describe('#_getDefaultConf', function() {
    ...
    it('will get default rpc port for livenet', function() {
      ...
      var quantisnetd = new QuantsisnetService(config);
      quantisnetd._getDefaultConf().rpcport.should.equal(9796);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var quantisnetd = new QuantsisnetService(config);
      quantisnetd._getDefaultConf().rpcport.should.equal(19796);
    });
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestQuantsisnetService = proxyquire('../../lib/services/quantisnetd', {
        fs: {
          readFile: readFile
        }
      });
      var quantisnetd = new TestQuantsisnetService(baseConfig);
      quantisnetd.spawnStopTime = 1;
      quantisnetd._process = {};
      quantisnetd._process.kill = sinon.stub();
      quantisnetd._stopSpawnedQuantsisnet(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from stopSpawnedQuantsisnet', function() {
      var quantisnetd = new QuantsisnetService(baseConfig);
      quantisnetd._loadSpawnConfiguration = sinon.stub();
      quantisnetd._stopSpawnedQuantsisnet = sinon.stub().callsArgWith(0, new Error('test'));
      quantisnetd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var quantisnetd = new TestQuantsisnetService(config);
      quantisnetd.spawn = {};
      quantisnetd._loadSpawnConfiguration = sinon.stub();
      quantisnetd._stopSpawnedQuantsisnet = sinon.stub().callsArgWith(0, null);
      quantisnetd.node.stopping = true;
      quantisnetd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will respawn quantisnetd spawned process', function(done) {
      ...
      quantisnetd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      quantisnetd._stopSpawnedQuantsisnet = sinon.stub().callsArg(0);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      ...
      quantisnetd.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Quantsisnet configuration options/);
      });
      done();
    });
  ...
  describe('#getMNList', function(){
    it('will call client masternode list and give result', function(done){
		    ...
		    client: {
			    masternodelist: function(type, cb){
			      switch (type){
				      ...
				      case "addr":
					      return cb(null, { result:
						      { '06c4c53b64019a021e8597c19e40807038cab4cd422ca9241db82aa19887354b-0': "108.61.209.47:9801",
							      'b76bafae974b80204e79858eb62aedec41159519c90d23f811cca1eca40f2e4c-1': "34.226.228.73:9801"}
					      });
				      case "status":
					      return cb(null, { result:
	    ...
	    quantisnetd.getMNList(function(err, MNList) {
		    ...
		    MNList[0].ip.should.equal("108.61.209.47:9801");
