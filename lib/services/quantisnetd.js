...
var QuantsisnetdRPC = require('quantisnetd-rpc');
 ...
 */
function Quantsisnet(options) {
  if (!(this instanceof Quantsisnet)) {
    return new Quantsisnet(options);
  }
...
util.inherits(Quantsisnet, Service);

Quantsisnet.dependencies = [];

Quantsisnet.DEFAULT_MAX_TXIDS = 1000;
Quantsisnet.DEFAULT_MAX_HISTORY = 50;
Quantsisnet.DEFAULT_MAX_UTXO = 1000;
Quantsisnet.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Quantsisnet.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9801;
Quantsisnet.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Quantsisnet.DEFAULT_SPAWN_RESTART_TIME = 5000;
Quantsisnet.DEFAULT_SPAWN_STOP_TIME = 10000;
Quantsisnet.DEFAULT_TRY_ALL_INTERVAL = 1000;
Quantsisnet.DEFAULT_REINDEX_INTERVAL = 10000;
Quantsisnet.DEFAULT_START_RETRY_INTERVAL = 5000;
Quantsisnet.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Quantsisnet.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Quantsisnet.DEFAULT_INSTANTSEND_FEE = 10000;
Quantsisnet.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Quantsisnet.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Quantsisnet.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Quantsisnet.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Quantsisnet.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Quantsisnet.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Quantsisnet.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Quantsisnet.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Quantsisnet.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Quantsisnet.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Quantsisnet.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Quantsisnet.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Quantsisnet.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Quantsisnet.prototype._initCaches = function() {
...
Quantsisnet.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Quantsisnet.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Quantsisnet.prototype.getPublishEvents = function() {
  return [
    {
...
Quantsisnet.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'quantisnetd/' + name, 'total:', this.subscriptions[name].length);
};

Quantsisnet.prototype.unsubscribe = function(name, emitter) {
...
Quantsisnet.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
...
Quantsisnet.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
 ...
 */
Quantsisnet.prototype.unsubscribeAddressAll = function(emitter) {
...
Quantsisnet.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Quantsisnet.DEFAULT_CONFIG_SETTINGS;
...
Quantsisnet.prototype._parseQuantsisnetConf = function(configPath) {
  var options = {};
...
Quantsisnet.prototype._expandRelativeDatadir = function() {
...
Quantsisnet.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using Quantsisnet config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseQuantsisnetConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseQuantsisnetConf(networkConfigPath));
  }
...
Quantsisnet.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
...
Quantsisnet.prototype._resetCaches = function() {
 ...
 * @private
 */
Quantsisnet.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Quantsisnet.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Quantsisnet.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Quantsisnet Daemon Ready');
          callback();
        });
      });
...
Quantsisnet.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9796
  };
  if (this.node.network === quantisnetcore.Networks.testnet) {
    networkOptions.rpcport = 19796;
  }
  return networkOptions;
};

Quantsisnet.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === quantisnetcore.Networks.testnet) {
...
Quantsisnet.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === quantisnetcore.Networks.testnet) {
...
Quantsisnet.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
...
Quantsisnet.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Quantsisnet.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Quantsisnet Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Quantsisnet.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Quantsisnet.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
...
Quantsisnet.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
...
Quantsisnet.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = quantisnetcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
...
Quantsisnet.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Quantsisnet.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Quantsisnet.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Quantsisnet.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Quantsisnet.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Quantsisnet Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Quantsisnet.DEFAULT_REINDEX_INTERVAL);

};

Quantsisnet.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Quantsisnet.prototype._stopSpawnedQuantsisnet = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
...
Quantsisnet.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedQuantsisnet(function(err) {
    if (err) {
      return callback(err);
    }
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Quantsisnet process unexpectedly exited with code:', code);
        ...
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            ...
            log.warn('Quantsisnet process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new QuantsisnetdRPC({
        protocol: 'http',
        host: '127.0.0.1',
...
Quantsisnet.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
Quantsisnet.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Quantsisnet configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.syncPercentage = function(callback) {
  var self = this;
...
Quantsisnet.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Quantsisnet.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Quantsisnet.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Quantsisnet.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Quantsisnet.prototype._getConfirmationsDetail = function(transaction) {
...
Quantsisnet.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Quantsisnet.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Quantsisnet.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Quantsisnet.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Quantsisnet.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Quantsisnet.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Quantsisnetcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Quantsisnet.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Quantsisnet.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Quantsisnet.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Quantsisnetcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Quantsisnet.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Quantsisnet.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Quantsisnet.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Quantsisnet.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Quantsisnet.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Quantsisnet.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Quantsisnet.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Quantsisnet.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Quantsisnet.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Quantsisnet.prototype.govCount = function (callback) {
  var self = this;
...
Quantsisnet.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Quantsisnet.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Quantsisnet.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Quantsisnet.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.getInfo = function(callback) {
  var self = this;
...
Quantsisnet.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantsisnet.prototype.stop = function(callback) {
