...
var BASE_PACKAGE = {
  description: 'A full Quantsisnet node build with Quantsisnetcore',
...
/**
 * Will create a directory and quantisnet.conf file for Quantsisnet.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createQuantsisnetDirectory(datadir, done) {
...
/**
 * Will create a base Quantsisnetcore Node configuration directory and files.
...
/**
 * Will setup a directory with a Quantsisnetcore Node directory, configuration file,
...
function create(options, done) {
  ...
  async.series([
    ...
    function(next) {
      // Setup the Quantsisnet directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createQuantsisnetDirectory(absDataDir, next);
      } else {
        next();
      }
    },
