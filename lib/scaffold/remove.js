...
/**
 * Will remove a service from quantisnetcore-node.json
...
/**
 * Will remove the Node.js service and from the quantisnetcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The quantisnetcore-node configuration directory
...
function remove(options, done) {
  ...
  var quantisnetcoreConfigPath = path.resolve(configPath, 'quantisnetcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(quantisnetcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a quantisnetcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to quantisnetcore-node.json
        removeConfig(quantisnetcoreConfigPath, service, next);
      });
    }, done
  );
}
