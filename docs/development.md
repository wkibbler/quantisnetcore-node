...
To develop bitcore-node-quantisnet:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-quantisnet.git
...
We now will setup symlinks in `bitcore-node-quantisnet` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-quantisnet
ln -s ~/bitcoind-rpc-quantisnet
```
...
cd devnode
mkdir node_modules
touch bitcore-node-quantisnet.json
touch package.json
```

Edit `bitcore-node-quantisnet.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-quantisnet/bin/bitcore-node-quantisnet start
```